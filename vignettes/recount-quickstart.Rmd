...
The gene ENTREZ ids are included in several objects in `recount`. With these ids you can find the gene symbols, gene names and other information. The `r Biocpkg('org.Hs.eg.db')` is useful for finding this information and the following code shows how to get the gene names and symbols.
...
We used the annotation information from the `r Biocpkg('TxDb.Hsapiens.UCSC.hg38.knownGene')` package version 3.1.3 to get the list of genes that have a Entrez `gene_id`. We then used the `r Biocpkg('org.Hs.eg.db')` package to get the gene symbols. For each gene, we also counted the total length of exonic base pairs for that given gene and stored this information in the `bp_length` column. You can see below this information for the `rse_gene` object included in `r Biocpkg('recount')`. The `rownames()` are the Entrez `gene_id`.
...
For an `rse_exon` object, the `rownames()` correspond to the Entrez `gene_id`. You can add a `gene_id` column if you are interested in subsetting the whole `rse_exon` object.
...
## Example subset
rse_exon_subset <- subset(rse_exon, subset = gene_id == '1')
rowRanges(rse_exon_subset)
```

The exons in a `rse_exon` object are those that have a Entrez `gene_id` and that were reduced as described in the `r Biocpkg('GenomicRanges')` package:
...
If you are interested in using another annotation based on hg38 coordinates you can do so using the function `coverage_matrix()` or by running [bwtool](https://github.com/CRG-Barcelona/bwtool/wiki) thanks to the BigWig files in `recount`.
...
exonMatrix <- coverage_matrix('SRP009615', 'chrY', unlist(exons_chrY),
    chunksize = 3000)
dim(exonMatrix)
head(exonMatrix)
...
The above solution works well for a small set of samples. However, `bwtool` is faster for processing large sets. You can see how we used `bwtool` with the annotation from `r Biocpkg('TxDb.Hsapiens.UCSC.hg38.knownGene')` package version 3.1.3 at [leekgroup/recount-website](https://github.com/leekgroup/recount-website). It's much faster than R and less memory intensive but requires several steps such as preparing an appropriate [exons BED file](https://github.com/leekgroup/recount-website/blob/master/genes/create_bed.R), [running bwtool](https://github.com/leekgroup/recount-website/blob/master/bwtool/run_bwtool.sh), [loading the exon counts](https://github.com/leekgroup/recount-website/blob/bc849a34e29d82dadc5d8f10508eaf573c26611d/rse/create_rse.R#L540-L546) and [summarizing them at the gene level](https://github.com/leekgroup/recount-website/blob/bc849a34e29d82dadc5d8f10508eaf573c26611d/rse/create_rse.R#L566-L568). Note that you will need to download the BigWig files first.
...
## Download all files (will take a while! It's over 6 TB of data)
...
Not everyone has over 6 terabytes of disk space available to download all the data from the `recount` project. However, thanks to [SciServer](http://www.sciserver.org/) you can access it locally via a Jupyter Notebook. If you do so and want to share your work, please let [SciServer](http://www.sciserver.org/) people know via Twitter at [IDIESJHU](https://twitter.com/IDIESJHU).
